...
def test_list_approved_artists(client, admin_headers, artist_pending, artist_approved):
    resp = client.get("/api/admin/artists?status=approved", headers=admin_headers)
    assert resp.status_code == 200
    data = resp.get_json()
...
def test_list_rejected_artists(client, admin_headers, artist_pending):
    ...
    resp = client.get("/api/admin/artists?status=rejected", headers=admin_headers)
    assert resp.status_code == 200
    data = resp.get_json()
...
def test_list_unsubmitted_artists_empty_ok(client, admin_headers):
    ...
    resp = client.get("/api/admin/artists?status=unsubmitted", headers=admin_headers)
    assert resp.status_code == 200
    assert isinstance(resp.get_json(), list)


def test_list_artists_invalid_status(client, admin_headers):
    resp = client.get("/api/admin/artists?status=does-not-exist", headers=admin_headers)
