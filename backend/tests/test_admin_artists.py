...
def test_admin_invalid_status_param(client, admin_headers):
    resp = client.get("/api/admin/artists?status=blabla", headers=admin_headers)
    assert resp.status_code == 400
    js = resp.get_json()
    assert "error" in js

def test_admin_approve_artist_flow(client, admin_headers, artist_pending, get_artist):
    # Approve
    ...
    # DB check
    updated = get_artist(artist_pending)
    assert updated.approval_status == "approved"

    # Idempotenz: nochmal approve -> weiterhin approved
    resp2 = client.post(f"/api/admin/artists/{artist_pending}/approve", headers=admin_headers)
    assert resp2.status_code == 200
    assert resp2.get_json()["status"] == "approved"

def test_admin_reject_artist_flow(client, admin_headers, artist_pending, get_artist):
...
def test_admin_reject_not_found(client, admin_headers):
    resp = client.post("/api/admin/artists/999999/reject", headers=admin_headers, json={"reason": "nope"})
    assert resp.status_code == 404